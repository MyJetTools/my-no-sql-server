...
pub async fn load_tables(app: Arc<AppContext>) {
    ...
    let tables_to_load = app.init_state.init(tables, app.logs.as_ref()).await;

    tokio::spawn(super::table_list_of_files_loader(
        app.clone(),
        tables_to_load,
    ));
...
async fn load_tables_spawned(app: Arc<AppContext>) {
    while let Some(table_to_load) = app.init_state.get_next_table_to_load().await {
        let mut sw = StopWatch::new();
        sw.start();
